...
from config import IMG_SIZE, BATCH_SIZE, FINAL_MODEL_PATH, PLOTS_DIR
...
def generate_confusion_matrix():
    ...
    predictions = model.predict(prep_test_ds, verbose=1)
...
def generate_sample_predictions():
    ...
    for i, (img_batch, label_batch) in enumerate(sample_ds.take(12)):
        pred = model.predict(preprocess_input(img_batch), verbose=0)
        pred_class = int(np.argmax(pred))
        confidence = np.max(pred) * 100
        true_class = int(label_batch.numpy()[0])
        
        # Convert tensor image to uint8 for display.
