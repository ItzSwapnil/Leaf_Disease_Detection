...
def main():
    parser = argparse.ArgumentParser(description="Evaluate leaf disease PyTorch model.")
    parser.add_argument(
        "--model-path",
        default=None,
    ...
    val_loader = build_dynamic_yolo_dataset(
        VAL_DIR, val_class_names, int(BATCH_SIZE), shuffle=False, use_yolo=not _skip_yolo
    )
    ...
    print(classification_report(y_true, y_pred, target_names=val_class_names, zero_division=0))
