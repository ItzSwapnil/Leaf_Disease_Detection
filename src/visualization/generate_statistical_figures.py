...
def _patch_vit_layer_init_for_compat() -> bool:
    ...
    Some saved models include `num_patches` and `num_positions` in the
    serialized config for `ViTPatchingAndEmbedding`, while newer KerasHub
    versions no longer accept those kwargs.
    """
    try:
...
def _infer_backbone(model, model_path: str | None = None) -> str:
    """Infer backbone from model metadata and/or selected model path."""
...
def generate_per_class_stability_plot(
    ...
    # Get class structure
    test_ds_iter = test_dataset_raw.unbatch().batch(BATCH_SIZE)
    class_counts = {}
    ...
    for seed in range(n_seeds):
        ...
        perturbed_ds = test_dataset_raw.map(
        ...
        y_pred = np.argmax(preds, axis=1)
        y_true = np.concatenate(
            [labels.numpy() for _, labels in test_dataset_raw], axis=0
        )

        # Compute one-vs-rest per-class F1 across the full set.
...
def main():
    ...
    print(f"[Statistical Suite] Backbone inference: {backbone_name}")
    ...
    try:
        generate_per_class_stability_plot(
            model,
            test_ds_raw,
            backbone_name,
            args.output_dir,
